...
public class ConSurplus extends JPanel
{
    ...
    {      
        ...
        arr2 = new ArrayList<String>(Arrays.asList("$" + df.format(Calculate.integrate(degree, coeff,0,sales, sell)),"$" + df.format(Calculate.integrate(degree, coeff,0,sales, 0)),"$" + df.format(Math.abs(Calculate.integrate(degree, coeff,0,sell, sell))),"$" + df.format(Math.abs(Calculate.integrate(degree, coeff,0,sell, sales)))));
        correct = arr2.get(0);
        for (int y = 4; y >= 1; y--)
        {
            choice = (int)(Math.random()*y);
